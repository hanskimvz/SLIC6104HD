...
static IPSDK_Int _JsonProtocolCallback(
	IPSDK_Int nJsonId,
	IPSDK_Int nCode,
	IPSDK_CPChar pResultJson,
	IPSDK_PVoid pUser)
{
	IPSDK_DEBUG("nJsonId:%d,nCode:%d,pResultJson:%s", nJsonId, nCode, (IPSDK_NULL == pResultJson) ? "" : pResultJson);
	return IPSDK_SUCCESS;
}
...
static IPSDK_Int _TransCallback(EN_IPSDK_TransmissionType enTransType, IPSDK_CPChar pDataBuf, IPSDK_Int nDataLen, IPSDK_PVoid pUser)
{
	CHECK_POINTER_ERR_RET(pDataBuf, IPSDK_FAILURE);
	IPSDK_DEBUG("type:%#x, datalen=%d", enTransType, nDataLen);
	return IPSDK_SUCCESS;
}


#define TEST_SEARCH_OPEN		1
#define TEST_LIVE_STREAM		1
#define TEST_LICENSE_DEL		0
#define TEST_JSONPROTOCOL_OPEN	0
#define TEST_NVR_OSD_OPEN		0
#define TEST_SNAPSHOT_OPEN		0
#define TEST_NOTIFY_OPEN		0
#define TEST_TRANS_OPEN			0
#define TEST_PRODUCT_TEST_OPEN	0
#define TEST_PLAYBACK_OPEN		0
#define TEST_AUDIOTALK_OPEN		0
...
int main(int argc, char* argv[])
{
	...
	char test_ipc[30] = {"192.168.1.188"};
	if (argc > 1){
	...
	IPSDK_HLOGIN hLogin = NETSDK_Login(
		test_ipc,
		9980,/// g_stSearchDevice[0].stLanInfo.nSessionPort,
		1680,/// g_stSearchDevice[0].stLanInfo.nStreamPort,
		"admin",///g_stSearchDevice[0].stUserInfo.strUserName,
		"admin", ///g_stSearchDevice[0].stUserInfo.strPassword,
		IPSDK_NULL);
	CHECK_POINTER_ERR_RET(hLogin, IPSDK_FAILURE);

#if  TEST_NVR_OSD_OPEN
	IPSDK_Int nOsdCnt = 0;
...
#if TEST_TRANS_OPEN
nRet = NETSDK_Trans(hLogin, _TransCallback, IPSDK_NULL);
if (IPSDK_SUCCESS == nRet)
{
	printf("Trans successfully!\r\n");
	EN_IPSDK_TransmissionType enTransType = EN_IPSDK_TRANSMISSION_TYPE_TP;
	IPSDK_Char strDataBuf[1024] = "ASCII for transparent transmission test!";
	NETSDK_TransSend(hLogin, enTransType, strDataBuf, strlen(strDataBuf));
}
#endif

#if TEST_PRODUCT_TEST_OPEN
...
#if TEST_JSONPROTOCOL_OPEN
	IPSDK_CPChar pParamJson = NULL;
	/// 1 EN_JSON_PROTOCOL_CMD_DEVICE_SearchOnvif = (416),	/// 远程控制设备启动ONVIF搜索
	nRet = NETSDK_JsonProtocol(
		hLogin,
		EN_JSON_PROTOCOL_CMD_DEVICE_SearchOnvif,
		0,
		0,
		_JsonProtocolCallback,
		pParamJson,
		NULL);

	/// 2 EN_JSON_PROTOCOL_CMD_DEVICE_SearchSdk = (417),	/// 远程控制设备启动私有协议搜索
	nRet = NETSDK_JsonProtocol(
		hLogin,
		EN_JSON_PROTOCOL_CMD_DEVICE_SearchSdk,
		0,
		0,
		_JsonProtocolCallback,
		pParamJson,
		NULL);
	/// 3 EN_JSON_PROTOCOL_CMD_DEVICE_ListChannel = (510),	/// 获取所有通道设备列表
	nRet = NETSDK_JsonProtocol(
		hLogin,
		EN_JSON_PROTOCOL_CMD_DEVICE_ListChannel,
		0,
		0,
		_JsonProtocolCallback,
		pParamJson,
